...
class MinMaxObserver(UniformQuantizationObserverBase):
    ...
    def forward(self, x_orig):
        ...
        if self.quantile:
            assert isinstance(self.quantile, float) or isinstance(self.quantile, torch.Tensor), f"type of quantile should be either float or torch.Tensor, got {type(self.quantile)}"
            x = torch.quantile(x, self.quantile, dim=0)
