...
def quantize_model(model, device, backend='default', sample_data=None):
    ...
    if backend == 'default':
        ...
        #     observer=quantizer.observer.MovingAverageMinMaxObserver.with_args(dtype=torch.qint8, quantile=0.95),
        #     observer=quantizer.observer.HistogramObserver.with_args(dtype=torch.qint8),
              quant_min=-128, quant_max=127, dtype=torch.qint8, qscheme=torch.per_tensor_affine, reduce_range=False)
        ...
        #     observer=quantizer.observer.PerChannelMinMaxObserver.with_args(dtype=torch.qint8,quantile=0.95), 
        #     observer=quantizer.observer.MovingAveragePerChannelMinMaxObserver.with_args(dtype=torch.qint8),
        #     observer=quantizer.observer.MovingAveragePerChannelMinMaxObserver.with_args(dtype=torch.qint8,quantile=0.95),
        ...
        b0_pre_bind_qconfig = quantizer.pre_bind(model, input_tensor=sample_data.to('cpu'), debug_mode=True, \
                                observer_scheme_dict={"weight_scheme": "MovingAverageMinMaxObserver", "activation_scheme": "MovingAverageMinMaxObserver"})
 
        # 2) assign qconfig to model
        model.qconfig = quantizer.QConfig(activation=bst_activation_quant, weight=bst_weight_quant, qconfig_dict=b0_pre_bind_qconfig)                               
