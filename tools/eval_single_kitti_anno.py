...
def parse_config():
    ...
    parser.add_argument('--cfg_file', type=str, default="../models/kitti/pointpillar.yaml", required=False,
                        help='specify the config for evaluation')
    parser.add_argument('--ckpt', type=str, default="../models/kitti/checkpoint_epoch_80.pth", required=False,
                        help='checkpoint to start from')
    parser.add_argument('--anno_folder', type=str, default="../example_data/single_inference_examples/kitti",
                        required=False,
                        help='annotation folder contains the raw data')
    parser.add_argument('--seq_id', type=int, default=0, required=False, help='sequence id of the raw data file')
    parser.add_argument('--sup_score', type=float, default=0.5,
                        help='detection confidence lower than it would not be visualized')
    parser.add_argument('--save_to_file', default=False, type=lambda x: (str(x).lower() == 'true'), help='save the images')
    parser.add_argument('--view_on_screen', default=True, type=lambda x: (str(x).lower() == 'true'),
...
def visualize(dt_anno, suppressed_score, sequence, anno_folder_path, show_img=True, save_img=False):
    """

    Args:
        dt_anno: annotation generated by eval_single_scene function
        suppressed_score: detection confidence lower than it would not be visualized
        sequence: int id for the frame
        anno_folder_path: annotation folder
        show_img: show img on screen or not. Disable it on server
        save_img: if not shown, enable to save the images instead
    ...
    if save_img:
        file_name = sequence + "_" + str(int(suppressed_score * 100)) + "_vis_res.jpg"
        file_location = os.path.join(anno_folder_path, file_name)
        cv2.imwrite(file_location, vis)
        print("image saved to " + file_location)
...
def main():
    ...
    visualize(dt_anno, args.sup_score, args.seq_id, args.anno_folder, show_img=args.view_on_screen,
              save_img=args.save_to_file)
