...
class CenterHeadOfficial(nn.Module):
    ...
    def forward_exp(self, spatial_features_2d):
        """
        ...
        return torch.cat((cls_preds, box_preds), 3)
    ...
    def forward(self, data_dict):
        """
        designed to cross validate the onnx results
        """
        if "MERGED_HEADS" not in self.model_cfg or self.model_cfg.MERGED_HEADS is False:
            # pth results
            spatial_features_2d = data_dict['spatial_features_2d']
            cls_preds = self.conv_cls(spatial_features_2d)
            box_preds = self.conv_box(spatial_features_2d)

            # print(box_preds.sum(), pth_box_preds.sum())
            cls_preds = cls_preds.permute(0, 2, 3, 1).contiguous()  # [N, H, W, C]
            box_preds = box_preds.permute(0, 2, 3, 1).contiguous()  # [N, H, W, C]
            self.forward_ret_dict['cls_preds'] = cls_preds
            self.forward_ret_dict['box_preds'] = box_preds
            ...
            # onnx results
            heads_out = data_dict['heads_out']

            # only for bstnnx
            heads_out = heads_out.permute(0, 2, 3, 1).contiguous()

            cls_preds = heads_out[:, :, :, 0:3]
            box_preds = heads_out[:, :, :, 3:11]
            # pth_sum = torch.cat((pth_cls_preds, pth_box_preds), 3)
            # print("shape", box_preds.shape, pth_box_preds.shape)
            # print("shape", heads_out.shape, pth_sum.shape)
            # print("sum", box_preds.sum(), pth_box_preds.sum())
            self.forward_ret_dict['cls_preds'] = cls_preds
            self.forward_ret_dict['box_preds'] = box_preds
            # print("shape", heads_out.shape, cls_preds.shape)
            # print("value", cls_preds, box_preds[:,23, 32, :])
        if self.training:
            targets_dict = self.assign_targets(
                gt_boxes=data_dict['gt_boxes']
            )
