...
def get_hesai_eval_result_kitti_style(gt_annos, dt_annos, current_classes, PR_detail_dict=None):        
    overlap_0_7 = np.array([[0.7, 0.5, 0.5, 0.5, 0.5, 0.5],
...
def clean_hesai_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['car', 'pedestrian', 'MotorBicycle', 'TrailerTruck', 'Bus']
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
    MAX_TRUNCATION = [0.15, 0.3, 0.5]
    # pedestrian and MotorBicycle objects further than this range will be ignores during evaluation
    SMALL_OBJECTS_MAX_Y = [50.0, 50.0, 50.0]
    SMALL_OBJECTS_MAX_X = [30.0, 30.0, 30.0]
    ...
    for i in range(num_gt):
        ...
        if gt_name == 'MotorBicycle'.lower() or gt_name == "pedestrian".lower():
            if abs(gt_anno['location'][i, 0]) > SMALL_OBJECTS_MAX_Y[difficulty] or \
                    abs(gt_anno['location'][i, 2]) > SMALL_OBJECTS_MAX_X[difficulty]:
                ignore = True
    ...
    for i in range(num_dt):
        ...
        # process detection as well
        if dt_anno["name"][i].lower() == 'MotorBicycle'.lower() \
                or dt_anno["name"][i].lower() == "pedestrian".lower():
            if abs(dt_anno['location'][i, 0]) > SMALL_OBJECTS_MAX_Y[difficulty] or \
                    abs(dt_anno['location'][i, 2]) > SMALL_OBJECTS_MAX_X[difficulty]:
                valid_class = -1
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        rets = clean_hesai_data(gt_annos[i], dt_annos[i], current_class, difficulty)
