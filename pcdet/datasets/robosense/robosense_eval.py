...
def get_robosense_eval_result_kitti_style(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
...
def clean_robosense_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['Vehicle', 'Pedestrian', 'MotorBicycle',
                   'Big_vehicle', 'Huge_vehicle', "Cone"]
    MIN_PTS_IN_GT = 5
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        rets = clean_robosense_data(gt_annos[i], dt_annos[i], current_class, difficulty)
