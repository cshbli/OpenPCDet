...
from ...utils import object3d_robosense, calibration_robosense, box_utils, common_utils
...
class RoboSenseDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.robo_infos = []
        self.calib = None
        self.include_robo_data(self.mode)

    def include_robo_data(self, mode):
        ...
        for info_path in self.dataset_cfg.INFO_PATH[mode]:
            ...
            with open(info_path, 'rb') as f:
                infos = pickle.load(f)
                robo_infos.extend(infos)

        self.robo_infos.extend(robo_infos)

        if self.logger is not None:
            self.logger.info('Total samples for RoboSense dataset: %d' % (len(robo_infos)))
    ...
    def get_label(self, idx):
        ...
        label_path = label_path[:-3] + "json"
        label_path = label_path.replace("/pcd", "/label")
        label_file = self.root_split_path / label_path
        # print(idx, str(label_file))
        assert label_file.exists(), str(label_file) + " not found"
        return object3d_robosense.get_objects_from_label(label_file)

    def get_calib(self):
        self.calib = calibration_robosense.Calibration()
        return self.calib
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        if 'annos' not in self.robo_infos[0].keys():
            return None, {}
        ...
        from ...datasets.robosense.robosense_eval import get_robosense_eval_result_kitti_style
        eval_det_annos = copy.deepcopy(det_annos)
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.robo_infos]
        ...
        ap_result_str, ap_dict = get_robosense_eval_result_kitti_style(eval_gt_annos, eval_det_annos, class_names)
    ...
    def __len__(self):
        if self._merge_all_iters_to_one_epoch:
            return len(self.robo_infos) * self.total_epochs

        return len(self.robo_infos)

    def __getitem__(self, index):
        np.set_printoptions(suppress=True)
        if self._merge_all_iters_to_one_epoch:
            index = index % len(self.robo_infos)

        info = copy.deepcopy(self.robo_infos[index])
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            image_info = {'image_idx': sample_idx, 'image_shape': self.get_image_shape()}
            ...
            if has_label:
                ...
                gt_boxes_img = box_utils.boxes3d_kitti_camera_to_imageboxes(
                    gt_boxes_camera, calib, image_shape=self.get_image_shape()
                )
...
def dump_annos():
    ...
    dataset_cfg = EasyDict(yaml.safe_load(open(
        "./tools/cfgs/dataset_configs/robosense_dataset.yaml")))
    ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
    dataset = RoboSenseDataset(dataset_cfg=dataset_cfg, class_names=['Car', 'Pedestrian'],
                           root_path=ROOT_DIR / 'data' / 'robosense', training=False)
    ...
    pickle.dump(gt_infos, open("./robo_infos.pkl", mode='wb'))


def create_robosense_annos(dataset_cfg, class_names, data_path, save_path, workers=8):
    ...
    robosense_infos_train = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    with open(train_filename, 'wb') as f:
        pickle.dump(robosense_infos_train, f)
    print('robosense_ info train file is saved to %s' % train_filename)

    dataset.set_split(val_split)
    robosense_infos_val = dataset.get_infos(num_workers=workers, has_label=True, count_inside_pts=True)
    with open(val_filename, 'wb') as f:
        pickle.dump(robosense_infos_val, f)
    print('robosense_ info val file is saved to %s' % val_filename)

    with open(trainval_filename, 'wb') as f:
        pickle.dump(robosense_infos_train + robosense_infos_val, f)
    print('robosense_ info trainval file is saved to %s' % trainval_filename)

    # dataset.set_split('test')
    # robosense_infos_test = dataset.get_infos(num_workers=workers, has_label=False, count_inside_pts=False)
    # with open(test_filename, 'wb') as f:
    #     pickle.dump(robosense_infos_test, f)
    # print('robosense_ info test file is saved to %s' % test_filename)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_robosense_infos':
        ...
        ROOT_DIR = (Path(__file__).resolve().parent / '../../../').resolve()
        create_robosense_annos(
            dataset_cfg=dataset_cfg,
            class_names=['Vehicle', 'Pedestrian', 'MotorBicycle', 'Big_vehicle', 'Huge_vehicle', "Cone"],
