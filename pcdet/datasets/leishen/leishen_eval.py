...
def get_leishen_eval_result_kitti_style(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    overlap_0_7 = np.array([[0.1, 0.1, 0.1, 0.1, 0.01],
                            [0.1, 0.1, 0.1, 0.1, 0.01],
                            [0.1, 0.1, 0.1, 0.1, 0.01]])
    # overlap_0_5 = np.array([[0.1, 0.1, 0.1, 0.1, 0.01],
    #                         [0.1, 0.1, 0.1, 0.1, 0.01],
    #                         [0.1, 0.1, 0.1, 0.1, 0.01]])
    ...
    class_to_name = {
        ...
        1: 'Truck',
        2: 'Bus',
        3: 'Non_motor_vehicles',
        4: 'Pedestrians'
    }
    # class names are hard coded in clean_data function as well
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            result += print_str((f"{class_to_name[curcls]}:{mAP3d[j, 0, i]:.4f}"))
            
            # ignore bbox, bev, aos in leishen
            '''
            result += print_str(
...
def clean_leishen_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['Car', 'Truck', 'Bus', 'Non_motor_vehicles', 'Pedestrians']
    MIN_PTS_IN_GT = 5
    DIS_HEIGHT_RATIO = [25, 36, 9999999999999]
    ...
    for i in range(num_gt):
        distance_gt = np.linalg.norm(gt_anno["gt_boxes_lidar"][i][0:3])
        height_gt = gt_anno["gt_boxes_lidar"][i][-2]
        bbox = gt_anno["bbox"][i]
        ...
        if gt_name == current_cls_name:
            ...
            valid_class = -1
        ignore = False
        #do not ignore objects in robosense
        '''
        if ((gt_anno["occluded"][i] > MAX_OCCLUSION[difficulty])
            ...
            ignore = True
        '''
        # do not ignore pedestrians in leishen
        '''
        # add difficuly level filtering
        if gt_name == 'Pedestrians':
            if distance_gt/height_gt > DIS_HEIGHT_RATIO[difficulty]:
                ignore = True
        else:
            if gt_anno['difficulty'][i] > difficulty:
                ignore = True
        '''
        if gt_anno['num_points_in_gt'][i] < MIN_PTS_IN_GT:
            ignore = True
    ...
    for i in range(num_dt):
        ...
        if height < MIN_HEIGHT[difficulty]:
            ...
            ignored_dt.append(-1)
        '''
        # ignore diffculty in leishen
        '''
        dis_to_cam = np.linalg.norm(dt_anno['location'][i])
        height = dt_anno['dimensions'][i][1]
        if dis_to_cam/height > DIS_HEIGHT_RATIO[difficulty]:
            ignored_dt.append(1)
        elif valid_class == 1:
            ignored_dt.append(0)
        else:
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty):
    ...
    for i in range(len(gt_annos)):
        rets = clean_leishen_data(gt_annos[i], dt_annos[i], current_class, difficulty)
